...
export function PlayerDetail() {
  ...
  useEffect(() => {
    const loadPlayerData = async () => {
      setLoading(true);
      try {
        const [playerData, maxStatsData] = await Promise.all([
          api.getPlayer(playerId, league),
          api.getMaxStats(league)
        ]);
  ...
  const p = {
    kda: normalizeValue(player.kda, 'kda'),
    csPerMin: normalizeValue(player.csPerMin, 'cs'),
    kp: normalizeValue(player.kp, 'kp'),
    wr: normalizeValue(player.wr, 'wr'),
    damage: normalizeValue(player.damage, 'dpm'),
    gold: normalizeValue(player.gold, 'gold'),
  };

  const max = maxStats ? {
    kda: normalizeValue(maxStats.kda, 'kda'),
    csPerMin: normalizeValue(maxStats.csPerMin, 'cs'),
    dpm: normalizeValue(maxStats.dpm, 'dpm'),
    gold: normalizeValue(maxStats.gold, 'gold'),
  } : null;
    ...
    datasets: [
      {
        ...
        data: max ? [
          (p.kda / max.kda) * 100,
          (p.csPerMin / max.csPerMin) * 100,
          p.kp, 
          p.wr, 
          (p.damage / max.dpm) * 100,
          (p.gold / max.gold) * 100,
        ] : [0, 0, 0, 0, 0, 0],
      ...
      tooltip: {
        callbacks: {
          label: function(context) {
            ...
            let realValue = 0;
            realValue = index === 0 ? p.kda :
                       index === 1 ? p.csPerMin :
                       index === 4 ? p.damage : p.gold;
