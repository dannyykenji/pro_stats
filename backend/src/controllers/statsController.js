...
const getTeamById = async (req, res) => {
  try {
    ...
    const players = playersResult.rows.map(player => ({
      ...
      damage: Math.floor(parseFloat(player.dpm)) || 0,
      gold: Math.floor(parseFloat(player.gold_per_min)) || 0,
    }));
...
const getTotalPlayersCount = async (req, res) => {
  try {
    ...
    const responseData = { total: count };
...
const getMaxStats = async (req, res) => {
  try {
    const { league } = req.query;
    const cacheKey = memoryCache.generateKey('max-stats', { league });

    const cachedMaxStats = memoryCache.get(cacheKey);
    if (cachedMaxStats) {
      return res.json(cachedMaxStats);
    }

    let whereClause = '';
    let values = [];
    if (league) {
      whereClause = 'WHERE league = $1';
      values = [league];
    }
    ...
    const maxStats = {
      kda: parseFloat(row.max_kda) || 5,
      csPerMin: parseFloat(row.max_cs) || 12,
      dpm: Math.floor(parseFloat(row.max_dpm)) || 900,
      gold: Math.floor(parseFloat(row.max_gold)) || 450,
    };

    memoryCache.set(cacheKey, maxStats, 30 * 60 * 1000);
...
module.exports = { getTeams, getTeamById, getPlayers, getPlayerById, getChampionStats, getTotalPlayersCount, getLastUpdateTime, getMaxStats };
